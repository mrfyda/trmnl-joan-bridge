...
## Forwarding to Terminus (BYOS)

Terminus reads device telemetry from request headers on `GET /api/display`. The
...
- `FW-Version` — risks the `Types::Version` check (→ 404) and feeds Terminus's
  firmware-update comparison; cosmetic here, since the shim never forwards
  Terminus's `firmware_url` to Joan (Joan only ever receives PV3 image frames).
...
**Tap → next playlist item:** Terminus's `/api/display` rotator advances the
playlist on every poll (unless the playlist is `manual`). So the shim treats any
touch as "advance": on a 76-byte packet it re-polls Terminus (rotating to the
